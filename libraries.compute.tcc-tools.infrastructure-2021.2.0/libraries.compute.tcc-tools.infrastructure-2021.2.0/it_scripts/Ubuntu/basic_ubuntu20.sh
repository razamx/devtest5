...
declare -a WS_PACKAGE_LIST=("nis" "autofs" "mc" "ssh" "ntp" "ntpdate" "apt-file" "cifs-utils" "nfs-common" "sshpass" "socat" "git" "smbclient" "net-tools")
declare -a CI_PACKAGE_LIST=("nis" "autofs" "mc" "ssh" "ntp" "ntpdate" "apt-file" "cifs-utils" "nfs-common" "sshpass" "socat" "git" "smbclient" "net-tools")
...
echo 'Acquire::http::Proxy "http://proxy-dmz.intel.com:911/";' >> /etc/apt/apt.conf.d/00aptitude
get_status "$STEP_NAME"
...
get_status "Configure Cryptographic policy"
service ssh restart
...
###################################################
#cat > /etc/lightdm/lightdm.conf.d/00-hide-user-list.conf << EOF
#[SeatDefaults]
...
#########################
wget https://gitlab.devtools.intel.com/mwchrist/multirealm_pam_krb5/raw/master/krb_install_linux_time_repos.sh
