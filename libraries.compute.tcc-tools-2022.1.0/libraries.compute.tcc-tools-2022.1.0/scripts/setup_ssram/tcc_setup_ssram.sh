...
CONFIGS_DIR=${CONFIGS_DIR:="$BASE_PATH/config"}
DEFAULT_TOOLS_PATH="/usr/share/tcc_tools/tools"

APPLY_CAPSULE=${APPLY_CAPSULE:="$BASE_PATH/apply_capsule_uefi.sh"}
CONTROL_RTCM=${CONTROL_RTCM:="$BASE_PATH/control_rtcm_uefi.sh"}
CONTROL_DRIVER="$BASE_PATH/control_tcc_driver.sh"
COMPARE_CACHE="$BASE_PATH/cache_configuration_comparator.py"
SELECT_CONFIG="$BASE_PATH/select_config.py"
CACHE_READ="tcc_cache_info_read"
DRIVER="tcc_buffer"

CPUS=($(for i in $(ls -d ${CONFIGS_DIR}/*/); do basename ${i}; done))
...
function configure() {
	...
	if [[ $MANUAL == 1 ]]; then
		...
		cpu_dir=$(python3 $SELECT_CONFIG -c ${CONFIGS_DIR})
		check_return_code "ERROR: Failed to find configuration"
	fi
	echo "Apply cache capsule:"
	capsules_dir="$cpu_dir/$MODE"
	bash -$- "$APPLY_CAPSULE" $capsules_dir
...
function check_cache_configuration() {
	...
	echo "$real_rtct" | python3 "$COMPARE_CACHE" --expected "$expected_rtct_path" --buffer --cache
	status=$?
	if [[ $status == 0 ]]; then
