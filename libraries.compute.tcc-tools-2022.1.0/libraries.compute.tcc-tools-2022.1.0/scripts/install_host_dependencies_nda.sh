...
#   * g++-10-multilib (for ittapi as build dependencies)
#   * cmake (for application build)
#   * json-c version 0.15 (for the cache allocation library build)
...
#   * checkinstall (required for libbpf, open62541 and libjson installation)
...
function install_itt {
    ...
    run_with_sudo_if_required apt-get -y install cmake g++-10-multilib
    check_return_code "\`apt-get install cmake g++-multilib\` failed"
    run_with_sudo_if_required update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-10 60 --slave /usr/bin/g++ g++ /usr/bin/g++-10
...
function install_json {
    run_with_sudo_if_required wget https://s3.amazonaws.com/json-c_releases/releases/json-c-0.15.tar.gz && \
    tar -xzf json-c-0.15.tar.gz && \
    mkdir json-c-0.15/build && \
    cd json-c-0.15/build && \
    cmake -DCMAKE_INSTALL_PREFIX=/usr/ .. && \
    make all test
    check_return_code "\`download json-c-0.15\` failed"
    ...
    run_with_sudo_if_required checkinstall -D --pkgname=json-c -y
    check_return_code "\`install json-c-0.15\` failed"
}
