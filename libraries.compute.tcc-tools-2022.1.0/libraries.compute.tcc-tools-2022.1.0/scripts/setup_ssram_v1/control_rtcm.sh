...
# Script to enable/disable SlimVMM (RTCM)
...
function usage() {
	echo "control_rtcm.sh - Enable/disable SlimVMM (RTCM)"
	echo "Usage:"
	echo "control_rtcm.sh enable|disable|status"
...
function add_boot_option() {
	find_boot_partition
	if [ -f '/boot/efi/EFI/BOOT/rtcm_boot.efi' ]; then
	...
	elif [ -f '/boot/efi/EFI/BOOT/ptcm_boot.efi' ]; then
		efibootmgr -c -l '\EFI\BOOT\ptcm_boot.efi' -d /dev/$disk -p $partition -L $BOOT_NAME
		check_return_code "ERROR: Failed to add boot option"
	else
		echo "ERROR: Can't find ptcm_boot.efi or rtcm_boot.efi"
		exit 1
	fi
}
